...
function anagramInProcess(word:string):string[] {
    this.output = [];
    this.traverse= function(str:string,perm='') {
        if (!str) this.output.push(perm)
...
const organize = (team: { id: string; boss: string; }[], boss='') => {
    let node = {}
